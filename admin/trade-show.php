	...
	$smsg=""; $emsg=""; 
		if(isset($_REQUEST['mode'])){
			if($_REQUEST['mode']=='del'){
				$addimag	=	mysqli_query($db,"update trade_show set status='3' where id='".$_REQUEST['id']."'");
				header("Location: trade-show.php");
		...
		$extension = explode("/", $_FILES["sliderimage"]["type"]);		
			...
			if(strtolower($imageFileType) == "jpg" || strtolower($imageFileType) == "png" || strtolower($imageFileType) == "jpeg" || strtolower($imageFileType) == "gif" ) 
				{
					$target_file = ("media/trade/" . $extension[0].rand(0,99999999).".".$extension[1]);
					if(move_uploaded_file($tmp, $target_file))
					{
						
						$addimag	=	mysqli_query($db,"insert into trade_show set title='".$title."', description='".$desc."', image='".$target_file."', status='1', date='".date('Y-m-d')."'");
						if($addimag){
					...
					$emsg	=	'Inavlid Image Type.';					
				}
			}else{
				$addimag	=	mysqli_query($db,"insert into trade_show set title='".$title."', description='".$desc."', status='1', date='".date('Y-m-d')."'");
				if($addimag){
						$smsg	=	'Information Added Successfully.';
					}
			}
		...
		$extension = explode("/", $_FILES["sliderimage"]["type"]);		
			...
			if(strtolower($imageFileType) == "jpg" || strtolower($imageFileType) == "png" || strtolower($imageFileType) == "jpeg" || strtolower($imageFileType) == "gif" ) 
				{
					$slinfo	=	mysqli_fetch_array(mysqli_query($db,"select * from trade_show where id='".$_REQUEST['id']."'"));
					unlink($slinfo['image']);
					...
					if(move_uploaded_file($tmp, $target_file))
					{
						...
						$addimag	=	mysqli_query($db,"update trade_show set title='".$title."', description='".$desc."', image='".$target_file."' where id='".$_REQUEST['id']."'");
						if($addimag){
				...
				$addimag	=	mysqli_query($db,"update trade_show set title='".$title."', description='".$desc."' where id='".$_REQUEST['id']."'");
				if($addimag){
						$smsg	=	'Information Updated Successfully.';
						header("Location: trade-show.php");
					}
...
?>
      ...
      <div class="col_3">
          ...
          <div class="panel panel-warning" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
            ...
            <div class="panel-body no-padding" style="display: block;">
              <table class="table table-striped">
                ...
                <tbody>
				<?php
					$pr_qry	=	mysqli_query($db,"select * from trade_show where status!='3' order by id desc");
				...
				?>
                  <tr>
					...
					<td>
						<a href="trade-show.php?mode=edit&id=<?php echo $pr_info['id']; ?>"><i class="fa fa-edit"></i></a>
						<!--<i class="fa fa-times"></i>
						<i class="fa fa-check"></i>-->
						<a href="trade-show.php?mode=del&id=<?php echo $pr_info['id']; ?>" onclick="return window.confirm('Are you sure?');"><i class="fa fa-trash-o"></i></a>
					</td>
                  </tr>
				  <?php }}else{?>
                  <tr>
		...
		<?php
				//echo $_REQUEST['mode'].$_REQUEST['id'];
			if(isset($_REQUEST['mode']))
			{		
				if($_REQUEST['mode']=='edit')
				{

					$qry	=	mysqli_query($db,"select * from trade_show where id='".$_REQUEST['id']."' and status!='3'");
          ...
          <form class="form-horizontal" method="post" enctype="multipart/form-data" >
			...
			<div class="form-group">
              ...
              <div class="col-sm-8">
				  ...
				  <a href="trade-show.php" class="btn btn-default">Reset</a>
			  </div>
            </div>
